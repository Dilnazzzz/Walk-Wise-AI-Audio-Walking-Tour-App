...
const MapScreen = ({ navigation }) => {
  ...
  const updateSaved = async () => {
    ...
    setSelectedPlaces([]);
    try {
      const docSnap = await getDocs(collection(FIRESTORE_DB, 'saved'));
      const attractions = [];
      docSnap.forEach((d) => attractions.push({ ...d.data() }));
  ...
  useEffect(() => {
    // initialize checkbox state when saved places change
    const initial = saved.reduce((acc, it) => ({ ...acc, [it.place]: 'square' }), {});
    setChosenn(initial);
  }, [saved]);
  ...
  const updateStatus = (data, index) => {
    if (chosenn[data.place] === 'check-square') {
      setChosenn((previousState) => {
        return { ...previousState, [data.place]: 'square' };
      });
      unselectPlace(data);
    } else {
      setChosenn((previousState) => {
        return { ...previousState, [data.place]: 'check-square' };
      });
      selectPlace(data);
    }
  };
  ...
  const showsummary = () => {
    ...
    // console.debug('Selected places', selectedPlaces);
    // console.debug('Summary', tourSummary);
  };
      ...
      <ScrollView className=" flex-1 px-4 py-4">
        ...
        {isLoading ? (
          ...
          <View>
              ...
              {saved?.length > 0 ? (
                  ...
                  {saved?.map((data, index) => {
                    return (
                      <View key={index} className="items-center flex-row space-x-6">
                        <Text className="flex-row items-center justify-center rounded-md py-2 px-2 text-#428288 text-[18px] font-bold">
                          {' '}
                          {data.place}{' '}
                        </Text>

                        <TouchableOpacity
                          onPress={() => updateStatus(data, index)}
                ...
                <>
                  <View className="w-full h-[400px] items-center space-y-2 justify-center ">
                    ...
                    <Text className="text-2xl text-[#428288] font-semibold mb-20">
                      {' '}
                      Ooops...No Data Found
                    </Text>
                  </View>
            ...
            <Text className="mt-6 text-[#428288] text-[24px] font-bold">Customize Your Tour</Text>
            <View className="flex-row items-center space-x-2 mt-4">
              <Text className="text-[#8C9EA6] text-[20px] font-bold">Preferred Duration</Text>
            </View>
            <View className="flex-row gap-2 items-center justify-start flex-wrap mt-4">
              {durations.map((n) => (
                <TouchableOpacity
                  onPress={() => {
                    tourSummary.duration = n;
                  }}
                  // key={n.key}
            ...
            <View className="flex-row items-center space-x-2 mt-4">
              <Text className="text-[#8C9EA6] text-[20px] font-bold">Desired Activities</Text>
            </View>
            <View className="flex-row gap-2 items-center justify-start flex-wrap mt-4">
              {activities.map((n) => (
                <TouchableOpacity
                  onPress={() => {
                    tourSummary.activities.push(n);
                  }}
                  // key={n.key}
                  className="px-2 py-1 rounded-md bg-emerald-100"
                >
            ...
            <View className="flex-row items-center space-x-2 mt-4">
              <Text className="text-[#8C9EA6] text-[20px] font-bold">Accessability Needs</Text>
            </View>
            <View className="flex-row gap-2 items-center justify-start flex-wrap mt-4">
              {accessability.map((n) => (
                <TouchableOpacity
                  onPress={() => {
                    tourSummary.accessability.push(n);
                  }}
                  // key={n.key}
                  className="px-2 py-1 rounded-md bg-emerald-100"
                >
            ...
            <View className="flex-row items-center space-x-2 mt-4">
              <Text className="text-[#8C9EA6] text-[20px] font-bold">Any other suggestions?</Text>
            </View>
            <View className="flex-row gap-2 items-center justify-start flex-wrap mt-4">
            ...
            <View className="inset-x-0  flex-row items-center justify-center shadow-lg py-6 bottom-5">
              <TouchableOpacity
                onPress={() => navigation.navigate('TourScreen', { stops: selectedPlaces })}
                className="flex-row items-center justify-center bg-[#06B2BE] rounded-md px-4 py-2 mt-4"
              >
